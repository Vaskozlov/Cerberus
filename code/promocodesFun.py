...
def enter_promocode(self):
    ...
    if self.message.text.isdecimal() and int(self.message.text) in promocodes.keys():
        ...
        if self.login == promocodes[num].creatorLogin:
            bot.send_message(self.chat_id, "Ты не можешь использовать свой же промокод. Попробуй ввести еще раз или напиши \"отменить\"")
            promocodes_lock.release()
            return

        elif self.login in invitedUsers and promocodes[num].words == 100:
            bot.send_message(self.chat_id, "Ты был уже приглашен другим пользователем. Ты не можешь использовать этот промокод.")
            promocodes_lock.release()
            return
        ...
        bot.send_message(self.chat_id, f"Данный промокод существует. Теперь у вас есть {self.config.paid_answers} слов", reply_markup=standart_keyboard)
        self.status = users_statuses.main_menu
