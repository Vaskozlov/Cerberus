...
with open("/Users/vasilijkozlov/Documents/logs/CerberousStart.log", mode = "a") as fin:
...
def help(chat_id: int):
    try:
        help_images = [
            telebot.types.InputMediaPhoto(open(f"data/images/example/{file}", "rb")) for file in os.listdir("data/images/example")
        ]
        bot.send_message(chat_id, "На фоторграфиях показаны примеры использования бота: 1-2 фотография регистрация, 3 вход в аккант бота, 4 изменение точности, 5 ввод промокода, 6 выполнение упражнения. Если вы хотите отменить как-либо действие напишите \"Отменить\"")
        bot.send_media_group(chat_id, help_images)
    except BaseException:
        bot.send_message(chat_id, "Возникла ошибка при отправке примеров использования бота. Попробуйте еще раз или обратитесь к администратору https://t.me/confidencess.")
