...
def proccess_exercise_loading(self):
    if self.status == users_statuses.world_amount_status:
        ...
        if len(data) > 0:
            ...
            keyb.add("Отменить")
            bot.send_message(self.chat_id, f"Выберите упражнение из списка или напишите номер любого.\n" + "".join(info), reply_markup=keyb)
        else:
...
def do_exercise(self):
    ...
    if self.message.text.isdecimal():
        ...
        if self.config.paid_answers < self.exercise_amount:
            bot.send_message(self.chat_id, f"Вы не можете сделать столько слов, потому что у вас есть всего {self.config.paid_answers} слов", reply_markup=standart_keyboard)
            self.status = users_statuses.main_menu
            return

        else:
            self.callback_message = bot.send_message(self.chat_id, "Упражнение выполняется... Прогресс: 0, ошибок: 0", reply_markup=None)
            ...
            try:
                ...
                bot.send_message(self.chat_id, f"Упражнение выполнено: {result[0]}/{self.exercise_amount}, ошибок: {result[1]}, у вас есть {self.config.paid_answers} слов", reply_markup=standart_keyboard)
                self.status = users_statuses.main_menu
                self.cerberous = None
                ...
                bot.send_message(self.chat_id, "Возникла ошибка, убедитесь в правильности номера упражнения", reply_markup=standart_keyboard)
                self.status = users_statuses.main_menu
